    ...
    if ($_GET["action"] == "login") {
        if (isset($_POST["email"]) && isset($_POST["pass"])) {
            ...
            if ($user_login_result == -1) {
                ...
                }elseif($_SESSION["tipo"] == 1){//CHECK-ADMIN-NOTIFICATIONS
                foreach($dbh -> getAllProducts() as $prodotto){
                    if($prodotto["quantità"] == 0){
                        $dbh->insertNotification($_SESSION["id"],"Il Prodotto : ".$prodotto["nome"]. " è terminato!");
                    }
                }
                }
            }  
